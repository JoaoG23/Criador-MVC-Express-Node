...
class {Classe}Controller implements IfluxocaixaController {
  async listarTodosPorPagina(req: Request, res: Response) {
    try {
      const { numero_pagina, quantidade_items_pagina } = req.query;
      const pagina = await fluxocaixaService.listarTodosPorPagina(
        numero_pagina,
  ...
  async listaPorId(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const fluxocaixa = await fluxocaixaService.listaPorId(id);
